...
- Repository description: `快手 MCP / Kuaishou MCP / Kwai MCP by SocialDataX for work/user search, work details, comments, comment replies, creator profiles, and creator works.`
...
- Command/stdio fallback: `npx -y mcp-remote https://mcp.socialdatax.com/kuaishou/mcp --header "Authorization: Bearer <SOCIALDATAX_API_KEY>"`
...
- Before submitting to directories, verify `https://mcp.socialdatax.com/kuaishou/.well-known/mcp/server-card.json` returns the Kuaishou server card, not the root XHS server card.
